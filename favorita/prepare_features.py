...
def feature_list(record):
    ...
    item_index = int(record['item_nbr'])
    year = int(record['yea'])
    month = int(record['mon'])
    day = int(record['day'])
    # week_of_year = dt.isocalendar()[1]
    day_of_week = int(record['date'])
    # transferred = record['transferred']
    family = int(record['family'])
    clas = int(record['class'])
    perishable = int(record['perishable'])
    city = int(record['city'])
    cluster = int(record['cluster'])
    typeO = int(record['type_x'])
    oil = record['dcoilwtico']
    # state_holiday = abc2int(record['StateHoliday'])
    # school_holiday = int(record['SchoolHoliday'])
    # num_customers = int(record['Customers'])
    promo = int(record['onpromotion'])
    state = int(record['state'])
    isHoliday = int(record['isHoliday'])
    isQuake = int(record['isQuake'])
    weekofyear = int(record['weekofyear'])
    # try:
...
for record in train_data:
    # if record['unit_sales'] != '0' and record['Open'] != '':
    fl = feature_list(record)
    train_data_X.append(fl)
    train_data_y.append(int(numpy.expm1(float(record['unit_sales']))))
...
print("Number of test datapoints: ",len(train_data_X), len(test_data_X))
